...
tab8_towide = tab_8_adt %>% select(ArbPersonID, ArbEncounterID, EventType, DaysFromDOBtoEventdate) %>% filter(EventType %in% c("Admission", "Discharge"))
tab8_wide <- tab8_towide %>% 
  pivot_wider(
...
analysis = left_join(analysis, tab8_wide)
sum(is.na(analysis$Admission))
sum(is.na(analysis$Discharge))
...
- it seems like mortality after discharge/during admission can be derived, but would need the admission/discharge window dates
...
Age at admission	AgeAtTimeOfAdmission	Table 2 Encounters -----------
Sex (male)	Sex	Table 1 Person -----------
Race	Race	Table 1 Person -----------
Ethnicity	Ethnicity	Table 1 Person -----------
In state (yes)	In_state_yn	Table 2 Encounters -----------
Died (yes) 	Death_YN	Table 1 ------------


## WAITING ON EMAIL FROM DONIA (UPDATED DATA) AND GUIDANCE ON WHICH WEIGHT/HEIGH TO USE (SEE TAB_3_CLEANING PROGRAM)
...
Mortality after discharge		? - questions about what date variable to use -- no more cdphe data included(?) in previous data
  30 day		
  90 day		
  1 year		
  2 year		
