...
for (i in 1:length(exposure_list$ExposureNumber)){
  print(exposure_list$ExposureName[i])
  print(exposure_list$ExposureNumber[i])
    data_a_converted <- count_exp(exposure_list$ExposureNumber[i], exposure_list$ExposureName[i], data_a_converted2)
}
...
for (name in exposure_groups) {
  for (num in 1:65) {
    data_a <- day_exp(num, name, data_a)
  }
}
...
data_a$Day_01_aki <- as.numeric(difftime(data_a$aki_episode_start_date, 
                                          data_a$start_date_for_ntmx_encoun, 
                                          units = "days"))

# The dataset `data_a` now includes the new variables from the count_exp and day_exp functions,
