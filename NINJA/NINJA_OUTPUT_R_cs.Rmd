...
data_d$NSAID_ketorolac_exp_pre = ifelse(data_d$Ketorolac_exp_pre == "Yes" | data_d$NSAID_exp_pre  == "Yes", 1, 0)
data_d$NSAID_ketorolac_exp_post = ifelse(data_d$Ketorolac_exp_post == "Yes" | data_d$NSAID_exp_post == "Yes", 1, 0)
...
# Figure DF
df_bar = df %>% select(record_id, initial_aki_stage, Vancomycin_exp, Zosyn_exp, Antivirals_exp, Contrast_exp, NSAID_ketorolac_exp, Calcineurin_inhibitors_exp)
...
vanc_alluvial = ggplot(vanc_long_df,
  ...
  ggtitle("Vancomycin Exposure") + theme_classic()
...
vanc_alluvial2 = ggplot(vanc_long_df,
  ...
  geom_stratum(aes(fill = status)) +
  geom_label(aes(fill = status),stat = "stratum", size = 3) +
  ggtitle("Vancomycin Exposure by initial AKI stage") + theme_classic()
